...
$sqlq = "SELECT vol, vol_name, YEAR(s_date) AS year, MONTH(s_date) AS month, YEAR(e_date) AS eyear, MONTH(e_date) AS emonth, RIGHT(YEAR(e_date), 2) AS teyear FROM vol WHERE vol  =".$totalrowsl;
        $stmtq = $conn->prepare($sqlq);
        $stmtq->execute();
        $resq = $stmtq->get_result();
	  $totalrowsl=0;
...
$sqlq = "SELECT * FROM vol";
    ...
    $sql = "SELECT * FROM accepted_thesis ORDER BY date_action DESC";
    $res = $conn->query($sql);
    $chck=0;
    if ($res->num_rows > 0) {
	...
	$quer="select * from thesis where id=".$ro['id']." && vol=".$totalrowsl;
